...
func New(ctx context.Context, config Config, zapLogger *zap.Logger) (*Database, error) {
	...
	if zapLogger == nil || zapLogger.Core() == nil {
	...
	gormInternalLogger := &ZapGormLogger{
		ZapLogger: log.With(zap.String("component", "gorm")),
		LogLevel:  logger.Info,
	}

	gormConnection, err := gorm.Open(postgres.Open(dsn), &gorm.Config{
		Logger: gormInternalLogger,
	})
	if err != nil {
		return nil, fmt.Errorf("%s: failed to open gorm connection: %w", operation, err)
	}

	if err := gormConnection.Use(tracing.NewPlugin(tracing.WithoutQueryVariables())); err != nil {
		return nil, fmt.Errorf("%s: failed to use gorm tracing: %w", operation, err)
	}

	sqlConnection, err := gormConnection.DB()
	if err != nil {
		return nil, fmt.Errorf("%s: failed to get sql.DB instance: %w", operation, err)
	}
